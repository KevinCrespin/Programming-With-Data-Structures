...
/* Why do you think I made you do Step 2?
 * So we can understand that our Generic Interface works to any data type
 * even when its components are changed. */
...
public class PersisterTest <T> {
	...
	void testSaveReadCourseFile() {
		...
		assertEquals(course1.meanGPA(students), p2.readObjectFromFile(fOCourse).meanGPA(students), 0.001);
	}
	...
	void testSaveListCourseToFile() {
		...
		for (int i= 0; i < courses.size(); i++) {
			...
			assertEquals(courses.get(i).meanGPA(students), (p2.readListFromFile(fLCourse)).get(i).meanGPA(students), 0.001);
		}
	}
